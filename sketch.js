...
function keyPressed(){
    if((keyCode === 32) && gameState ==="launched"){
        ...
        if(birds.length>0 ){  
            //birds[birds.length-1].trajectory=[];
            //console.log(birds[birds.length-1]);
            //birds.pop();  //when this removes last bird, line 178 throws error and it should work only if length>0. So, pop should be given outside so that when it removes last bird, length=0 and 178 will not run and throw error          
            console.log("after pop"+birds.length);
            ...
            gameState="over"; //to prevent lines 170,171 from running after birds.length=0 as thet will run only when gameState = launched
        }
